 ...
 * @param {Usuario} res usuario encontrado
 */
export const getUser = async (req, res) => {
    try {
        const user = await pool.query(`SELECT * FROM users WHERE id = ${req.params.id}`);
        if (user.rowCount === 0) return res.status(404).json({ message: 'User not found' }); // 404 Not Found
...
export const createUser = async (req, res) => {
    try {
        ...
        const user = await pool.query(`INSERT INTO users (name) VALUES ('${req.body.name}') RETURNING *`);
        if (!user) return res.status(400).json({ message: 'User not created' }); // 400 Bad Request
        res.json(user.rows[0]);
    } catch (error) {
        return res.status(500).json({ message: error.message }); // 500 Internal Server Error
    }
...
export const updateUser = async (req, res) => {
    try {
        const user = await pool.query(`UPDATE users SET name = '${req.body.name}' WHERE id = ${req.params.id} RETURNING *`);
        if (!user) return res.status(400).json({ message: 'User not found' }); // 400 Bad Request
        res.json(user.rows[0]);
    } catch (error) {
        return res.status(500).json({ message: error.message }); // 500 Internal Server Error
    }
...
export const deleteUser = async (req, res) => {
    try {
        const user = await pool.query(`DELETE FROM users WHERE id = ${req.params.id} RETURNING *`);
        if (!user) return res.status(400).json({ message: 'User not found' }); // 400 Bad Request
        res.json(user.rows[0]);
    } catch (error) {
        return res.status(500).json({ message: error.message }); // 500 Internal Server Error
